...
then
  ...
  if [[ $* == *--virtual* ]]
  then
    python3 moench_tango_control_server.py moench -ORBendPoint giop:tcp:localhost:1234 -nodb -dlist moench/virtual/control && python3 moench_tango_acquire_server.py moench -ORBendPoint giop:tcp:localhost:1235 -nodb -dlist moench/virtual/acquire && echo ">>>> Virtual MOENCH tango server enviroment started"
  else
